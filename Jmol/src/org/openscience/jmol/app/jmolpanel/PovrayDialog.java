...
public class PovrayDialog extends JDialog {
  ...
  /**
   * Creates a dialog for getting info related to output frames in povray
   * format.
   * 
   * @param f
    ...
    */
    ...
    for (int power = 0; power < 8; power++) {
      mosaicPreviewComboStart
          .addItem(Integer.toString((int) Math.pow(2, power)));
    }
  ...
  /**
   * Sets the output image dimensions. Setting either to &lt;= 0 will remove the
   * height and width specification from the commandline- the resulting
   * behaviour depends on povray!
   * 
   * @param imageWidth
  ...
  /**
   * Generates a commandline from the options set for povray path etc and sets
   * in the textField.
   * 
   * @return command line
   */
  ...
  private String getINI() {
    ...
    // Mosaic preview options
    if ((mosaicPreviewCheck != null) && (mosaicPreviewCheck.isSelected())) {
      data.append("Preview_Start_Size="
          + mosaicPreviewComboStart.getSelectedItem() + "\n");
