...
public class JSONReader extends AtomSetCollectionReader {
   ...
   * 
   * leading "m" instead of "mol" indicates that a scaling of [1/20, -1/20, 1/20] is required because this is from ChemDoodle.
   * 
  ...
  public void initializeReader() throws Exception {
    ...
    readLine();
    isChemDoodle = checkFilterKey("CD") || line.startsWith("{\"m\"");
    readAtoms(getSection("a"));
    readBonds(getSection("b"));
    continuing = false;
  }

  private String[] getSection(String key) {
    String data = PT.split(line, "\"" + key + "\":[{")[1];
    data = data.substring(0, data.indexOf("]")) + ":";
    return PT.split(data.replace('"', ' ').replace('}', ' ').replace(',', ':'),
        "{");
  }
  ...
  private void readAtoms(String[] atoms) throws Exception {
    ...
    for (int i = 0; i < atoms.length; ++i) {
      ...
      atom.elementSymbol = lxyz[1].trim();
      float x = parseFloatStr(lxyz[3].trim());
      float y = parseFloatStr(lxyz[5].trim());
      float z = parseFloatStr(lxyz[7].trim());
      if (isChemDoodle) {
        x /= 20;
        y /= -20;
        z /= 20;
  ...
  private void readBonds(String[] bonds) throws Exception {
    for (int i = 0; i < bonds.length; ++i) {
      ...
      if (b.indexOf("o") < 0)
        b += " o : 1";
      String[] lxyz = b.split(":");
      int atomIndex1 = parseIntStr(lxyz[1].trim());
      int atomIndex2 = parseIntStr(lxyz[3].trim());
      int order = parseIntStr(lxyz[5]);
