...
//Bob Hanson hansonr@stolaf.edu 4/18/2017 7:44:43 AM
  ...
  private boolean couldBeChiralAtom(Node a) {
    ...
    return ((Math.abs(d) < TRIGONALITY_MIN) == mustBePlanar); // arbitrarily set 
  }
   ...
   *         | STEREO_ra | STEREO_sa}
   */
  private int getAxialOrEZChirality(Node a, Node b, boolean isAxial, int ruleMax) {
    htPathPoints = new Hashtable<String, Integer>();
...
//        System.out.println("$ draw " + lonePair + " // " + atom );
     ...
     * we are looking at {1:SRRR, 2:RSSS, 2:RSSS}, where 1 and 2 are priorities, with
     * 1 being higher priority than 2, then the reference designation is S, not R.
     * 
...
//      System.out.println(this + " creating aux " + s);
    ...
    private void checkPseudoHandedness(int[] iab, int[] indices) {
      ...
      if (auxPseudo == null) {
        ...
        atom1 = (CIPAtom) clone();
        atom1.atoms[indices[ia]] = new CIPAtom().create(null, atom1, false, isAlkene);
        atom1.atoms[indices[ib]] = new CIPAtom().create(null, atom1, false, isAlkene);
        atom1.addReturnPath(null, this);
      } else {
        atom1 = auxPseudo;
      }
