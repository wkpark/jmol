...
class Jmol extends JPanel {
	...
	static {
		...
		ujmoldir.mkdirs();
		JmolResourceHandler
				.initialize("org.openscience.jmol.Properties.Jmol");
	...
	public static void main(String[] args) {
		...
		**/
		...
		try {
			...
			if (script != null) {
				try {
					...
					RasMolScriptHandler scripthandler =
						new RasMolScriptHandler(window);
		...
		try {
			...
			consoleframe.getContentPane()
					.add(new JScrollPane(consoleTextArea),
			...
			try {
				...
				if (typeHint.equals("PDB")) {
					ChemFileReader reader =
						new PDBReader(new InputStreamReader(is));
					cf = reader.read();
				} else if (typeHint.equals("CML")) {
					ChemFileReader reader =
						new CMLReader(new InputStreamReader(is));
					cf = reader.read();
				} else if (typeHint.equals("XYZ (xmol)")) {
					ChemFileReader reader =
						new XYZReader(new InputStreamReader(is));
					cf = reader.read();
				} else {
	...
	protected JMenuItem createMenuItem(String cmd, boolean isRadio) {
		...
		if (isRadio) {
			...
			if (checked != null) {
				...
				mi = new JCheckBoxMenuItem(jrh.getString(cmd + labelSuffix),
						c);
			} else {
			...
			if (radiogroup != null) {
				...
				for (int j = 0; j < radioKeys.length; j++) {
					JRadioButtonMenuItem mi =
						(JRadioButtonMenuItem) createMenuItem(radioKeys[j],
							true);
					menu.add(mi);
					bg.add(mi);
	...
	private Action[] defaultActions = {
		new NewAction(), new OpenAction(), new CloseAction(),
		new SaveAction(), new PrintAction(), new ExportAction(),
		new ExitAction(), new AboutAction(), new WhatsNewAction(),
		new UguideAction(), new AtompropsAction(), undoAction, redoAction,
		new ConsoleAction(), chemicalShifts, new RecentFilesAction(),
		new PovrayAction(), new ScriptAction()
	};
				...
				if (theFile != null) {
					try {
						...
						if (ft.getType().equals("XYZ (xmol)")) {
							XYZSaver xyzs = new XYZSaver(getCurrentFile(),
												os);
							xyzs.writeFile();
				...
				if (theFile != null) {
					try {
						...
						if (it.getType().equals("JPEG")) {
							...
							JpegEncoder jc = new JpegEncoder(eImage, qual,
												 os);
							jc.Compress();
