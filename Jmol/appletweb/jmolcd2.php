...
// ChemDoodle server using JmolData.jar in silent restricted mode
// requires JmolData.jar version 12.3.22
// Bob Hanson hansonr@stolaf.edu 4/8/2012-4/13/2012
...
if ($call == "getImageForFileLoad" || $call == "getImageFromDatabase") {
  ...
  // we use select molecule=1 because some salts are retrieved by NCI, and we want to ignore them
  // it's not perfect.

	$query = getValueSimple($values, "query", "morphine");
	$cmd = $cmd.'"load \\"'.$database.$query.'\\"'.$postLoad.';write CD"';  
	exec($cmd, $result);
	$result = implode($result);
	$output = "{\"jmolcd_message\":\"$call - $query\",\"values\":$values,\"content\":$result}";
