...
public class ServerToClientProtocol extends ClientProtocol {
  ...
  /*
   * public int getGroupID(Location location) { GetGroupID msg = new
   * GetGroupID(location.getX(), location.getY()); GroupIDMessage response =
   * msg.send(_in, _out); return response.getGroupID(); }
   */
  ...
  public void processEvents(int groupID, Vector events) throws IOException {
    for (int i = 0; i < events.size(); i++) {
      ...
      // System.out.println("SeverToClient: " + groupID + " " +
      // Integer.toBinaryString(groupID));
      _bufferOut.writeInt(groupID);
  ...
  /**
	 ...
	 */
  /*
   * public void processEvents(int groupID, Vector events) { ProcessEvents msg =
   * new ProcessEvents(groupID, events.toArray(new Event[0])); msg.send(_in,
   * _out); }
   */
