...
public class PovrayStyleWriter {
	 ...
	 **/
	public void writeAtomsAndBondsDeclare(BufferedWriter w, ChemFrame cf)
			throws IOException {
		...
		for (int j = 0; j < numTypes; j++) {
			BaseAtomType at =
				cf.getAtomAt(indexOfExampleAtom[j]).getBaseAtomType();
			java.awt.Color c = at.getColor();
			String def = "#declare " + at.getName()
							 + "_PovrayStyleWriter = sphere{<0,0,0>,"
							 + at.getVdwRadius() + " texture{pigment{"
							 + povrayColor(c) + "}finish{Shiny}}}\n";
			w.write(def);
		}
	}
	...
	public void writeAtom(BufferedWriter w, int atomIndex, ChemFrame cf)
		...
		String st = ("object{ " + getAtomName(cf, atomIndex)
						+ "_PovrayStyleWriter translate<"
