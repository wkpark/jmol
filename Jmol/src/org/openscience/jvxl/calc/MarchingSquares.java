...
public class MarchingSquares {
  ...
  public int addContourVertex(int x, int y, int z, Point3i offsets, Point3f vertexXYZ,
    ...
    y += offsets.y;
    z += offsets.z;
    int vPt = voxelReader.addVertexCopy(vertexXYZ, value);
...
//    if (logMessages)
  ...
  private void processOneQuadrilateral(int insideMask, float cutoff,
    ...
    for (int iEdge = 4; --iEdge >= 0;) {
      ...
      pixelPointIndexes[iEdge] = voxelReader.addVertexCopy(contourPoints[iEdge], cutoff);
    }
  ...
  private void calcPixelVertexVectors() {
    for (int i = 4; --i >= 0;)
      planarMatrix.transform(pixelVertexVectors[i] = new Vector3f(),
          squareVertexVectors[i]);
  }
