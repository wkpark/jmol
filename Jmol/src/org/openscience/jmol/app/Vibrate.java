...
public class Vibrate extends JDialog implements ActionListener,
   ...
   * Creates a dialog.
   */
  public Vibrate(JmolViewer viewer, JFrame f) {
    ...
    vibPanel
        .setBorder(new TitledBorder(JmolResourceHandler.getInstance()
          .getString("Vibrate.vibrationLabel")));
    ...
    progressPanel
        .setBorder(new TitledBorder(JmolResourceHandler.getInstance()
          .getString("Vibrate.progressLabel")));
    ...
    savePanel
        .setBorder(new TitledBorder(JmolResourceHandler.getInstance()
          .getString("Vibrate.saveLabel")));

    JButton snmb =
      new JButton(JmolResourceHandler.getInstance()
        .getIcon("Vibrate.saveImage"));
    ...
    JLabel sgl =
      new JLabel(JmolResourceHandler.getInstance()
        .getString("Vibrate.movieTooltip"));
    c.gridx = 1;
    c.gridwidth = GridBagConstraints.REMAINDER;
  ...
  private void createVibration() {
    ...
    vibFile = new ChemFile(viewer);
    for (int n = 0; n < numberFrames; ++n) {
      int numberVerticies = inputFrame.getAtomCount();
      ChemFrame newFrame = new ChemFrame(viewer, numberVerticies);
  ...
  public void setVisible(boolean b) {
    ...
    if (b) {
      vibrateAction.setEnabled(false);
      viewer.setClientFile("Vibrate", vibFile);
      currentFrame = 0;
      ...
      stop();
      if (inputFile != null) {
        viewer.setClientFile("Vibrate", inputFile);
      }
      vibrateAction.setEnabled(true);
      restoreConflictingActions();
    }
