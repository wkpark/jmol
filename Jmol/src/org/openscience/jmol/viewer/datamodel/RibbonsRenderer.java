...
class RibbonsRenderer extends McpsRenderer { // not current for Mcp class
  ...
  void renderMcpschain(Mcps.Mcpschain mcpschain) {
    Ribbons.Rchain rchain = (Ribbons.Rchain)mcpschain;
    polymerCount = rchain.polymerCount;
    ...
    render1Chain(rchain.polymerCount,
                 rchain.polymerGroups,
                 rchain.centers,
                 rchain.vectors,
                 rchain.mads,
                 rchain.colixes);
  ...
  void render1Chain(int polymerCount,
    ...
    render2Strand(polymerCount, groups, mads,
                  colixes, screensTop, screensBottom);
  }
  ...
  void render2Strand(int polymerCount, PdbGroup[] groups, short[] mads,
    ...
    for (int i = polymerCount; --i >= 0; ){
      if (mads[i] > 0){
        render2StrandSegment(polymerCount, groups[i], colixes[i],
                             mads, screensTop, screensBottom, i);}}
  }
