			...
			if (word.equals("load")) {
				if (st.hasMoreElements()) {
					...
					throw new RasMolScriptException(
							"Error: omitted parameter.");
				}
			} else if (word.equals("script")) {
				if (st.hasMoreElements()) {
					String file = (String) st.nextElement();
					try {
						File script = new File(program.getUserDirectory(),
										  file);
						if (script.exists()) {
				...
				if (st.hasMoreElements()) {
					...
					while (selectedAtoms.hasMoreElements()) {
						int atom =
							((Integer) selectedAtoms.nextElement())
								.intValue();
				...
				throw new RasMolScriptException("Unrecognized command: "
						+ command);
			}
		}
	}
	...
	private void setParam(String param, String value)
		...
		if (param.equals("shadows")) {
			...
			try {
				...
				throw new RasMolScriptException(
						"Error: cannot set autorefresh.");
			}
		} else {
			throw new RasMolScriptException("Unrecognized parameter: "
					+ param);
		}
	}
		...
		try {
			...
			if (!b) {
				program.display.settings
						.setLabelMode(DisplaySettings.NOLABELS);
			} else {
			...
			if (type.equals("%a")) {
				program.display.settings
						.setLabelMode(DisplaySettings.SYMBOLS);
			} else if (type.equals("%i")) {
				program.display.settings
						.setLabelMode(DisplaySettings.NUMBERS);
			}
		}
	}
		...
		if (value.equals("all") || value.equals("*")) {
			...
			while (st.hasMoreElements()) {
				...
				try {
					...
					throw new RasMolScriptException(
							"Error: invalid expression: " + subexpr);
				}
			}
		} else {
			try {
	...
	**/
	...
	private void setColor(String object, String value)
		...
		if (object.equals("atom")) {
			...
			while (selectedAtoms.hasMoreElements()) {
				...
				program.display.md.getAtomAt(atom
						- 1).setColor(this.getColor(value));
			}
			...
			throw new RasMolScriptException("Error: unknown object: "
					+ object);
		}
	}
	...
	private boolean checkBoolean(String value) throws RasMolScriptException {
		...
		if (value.equals("true")) {
			...
			throw new RasMolScriptException("Unrecognized boolean value: "
					+ value);
		}
	}
