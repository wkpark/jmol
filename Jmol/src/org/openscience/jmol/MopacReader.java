...
class MopacReader implements ChemFileReader {
	...
	public ChemFile read() throws Exception {
		...
		while (input.ready()) {
			...
			if (line.indexOf("TOTAL ENERGY") >= 0) {
				frameInfo = line.trim();
			} else if (line.indexOf("ORIENTATION OF MOLECULE IN FORCE CALCULATION") >= 0) {
				for (int i=0; i < 3; ++i) {
					line = input.readLine();
				}
				if (frame != null) {
	...
	void readCoordinates(ChemFrame mol) throws IOException, Exception {
		...
		while (input.ready()) {
			...
			} else throw new Exception("Error reading coordinates");
			if (token.nextToken() == StreamTokenizer.TT_NUMBER) {
				x = token.nval;
			} else throw new Exception("Error reading coordinates");
			if (token.nextToken() == StreamTokenizer.TT_NUMBER) {
		...
		while (line.indexOf("ROOT NO") >= 0) {
			...
			for (int i=0; i < mol.getNvert(); ++i) {
				...
				for (int j=0; j < currentFreqs.length; ++j) {
					...
					} else throw new Exception("Error reading frequencies");
				}
