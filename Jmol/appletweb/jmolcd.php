...
// Bob Hanson hansonr@stolaf.edu 4/8/2012
...
//$values = '{"call":"getMoleculeFromDatabase","query":"morphine"}';
...
if ($call == "getRawDataFromDatabase") {
  ...
  $format = getValueSimple($values, "format", "MOL");
  ...
  // it's not perfect.
  $cmd = '/home/mscs/common/bin32/java -Djava.awt.headless=true -jar JmolData.jar -iRJ "load \\"\\$'.$query.'\\"; n = ({molecule=1}.length < {molecule=2}.length ? 2 : 1); select molecule=n;write '.$format.'"';
  
  exec($cmd, $result);
  $output = implode("\n",$result);
} else if ($call == "getMoleculeFromDatabase") {
  $query = getValueSimple($values, "query", "morphine");
  $format = getValueSimple($values, "format", "CD");
  // note: ChemDoodle coordinates in JSON are factored by [20, -20, 20]
  // we use select molecule=1 because some salts are retrieved by NCI, and we want to ignore them
  // it's not perfect.
  $cmd = '/home/mscs/common/bin32/java -Djava.awt.headless=true -jar JmolData.jar -iRJ "load \\"\\$'.$query.'\\"; n = ({molecule=1}.length < {molecule=2}.length ? 2 : 1); select molecule=n;write '.$format.'"';
