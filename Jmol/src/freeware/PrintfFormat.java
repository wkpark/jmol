		...
		while (e.hasMoreElements()) {
			...
			if (c == '\0') {
				...
				if (o[i] instanceof Byte) {
					...
					sb.append(cs
							.internalsprintf(((Short) o[i]).shortValue()));
				} else if (o[i] instanceof Integer) {
					sb.append(cs
							.internalsprintf(((Integer) o[i]).intValue()));
				} else if (o[i] instanceof Long) {
					sb.append(cs.internalsprintf(((Long) o[i]).longValue()));
				} else if (o[i] instanceof Float) {
					sb.append(cs
							.internalsprintf(((Float) o[i]).floatValue()));
				} else if (o[i] instanceof Double) {
					sb.append(cs
							.internalsprintf(((Double) o[i]).doubleValue()));
		...
		while (e.hasMoreElements()) {
			...
			if (c == '\0') {
				...
				if (x instanceof Byte) {
					...
					sb.append(cs
							.internalsprintf(((Character) x).charValue()));
	...
	private class ConversionSpecification {
		...
		private char[] fFormatDigits(double x) {
			...
			if (n1In + expon <= 0) {
				...
				if (alternateForm || !precisionSet || (precision != 0)) {
					ca4[1] = '.';
					for (i = 0, j = 2; i < Math.min(p, ca3.length);
							i++, j++) {
						ca4[j] = ca3[i];
					}
					for (; j < ca4.length; j++) {
				...
				for (i = 0; i < Math.min(n1In + expon, ca3.length);
						i++, j++) {
					ca4[j] = ca3[i];
				}
		...
		private String printXFormat(int x) {
			...
			if (x == Integer.MIN_VALUE) {
				...
				String t = Integer.toString((~(-x - 1)) ^ Integer.MIN_VALUE,
							   16);
				switch (t.length()) {
				case 1 :
					sx = "8000000" + t;
