...
public class DisplayPanel extends JPanel
	...
	class MyMotionAdapter extends MouseMotionAdapter {
		...
		public void mouseDragged(MouseEvent e) {
			...
			if (mode == ROTATE) {
				...
				*/
				double xtheta = (y - prevy)
									* (2.0 * Math.PI / getSize().width);
				double ytheta = (x - prevx)
			...
			if (mode == ZOOM) {
				float xs = 1.0f + (float) (x - prevx)
							   / (float) getSize().width;
				float ys = 1.0f + (float) (prevy - y)
	...
	public Action[] getActions() {
		...
		Action[] defaultActions = {
			...
			topAction, bottomAction, rightAction, leftAction,
			aquickdrawAction, ashadingAction, awireframeAction,
			bquickdrawAction, bshadingAction, blineAction, bwireframeAction,
			plainAction, symbolsAction, typesAction, numbersAction,
			bondsAction, atomsAction, vectorsAction, hydrogensAction,
