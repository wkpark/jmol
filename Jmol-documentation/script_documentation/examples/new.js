...
useSigned = true;//(docsearch.indexOf("SIGNED")>=0)
...
function getscriptlink(i,isul,addNumber){
 ...
 for(var j=0;j<S.length;j++){
	...
	if(S[j].indexOf("###")>=0){
		nTopics--;
		S[j]=S[j].replace(/\#\#\#/,"<h3><span>" + (nTopics <= nFirst ? "<a name=\"topic"+nTopics+"\" id=\"topic"+nTopics+"\"><br /><a href='" + document.location.href.split("#")[0].split("?")[0] + "?topic=" + nTopics + "'>"   + nTopics + ".</a></a> " : "")).replace(/\#\#\#/,"</span></h3>")
		if(S[j].indexOf("<br>")>=0)
