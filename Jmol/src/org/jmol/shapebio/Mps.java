...
public abstract class Mps extends Shape {
  ...
  class Mpsmodel {
    ...
    Mpsmodel(Mps shape, Model model) {
      mpspolymers = new MpsShape[model.getBioPolymerCount()];
      this.modelIndex = model.getModelIndex();
      for (int i = mpspolymers.length; --i >= 0;) {
        (mpspolymers[i] = allocateMpspolymer((BioPolymer)model.getBioPolymer(i)))
            .setShape(shape);
      }
    }
  ...
  abstract class MpsShape {
    ...
    MpsShape(BioPolymer polymer, int madOn,
   ...
   void setModelClickability() {
      ...
      for (int i = monomerCount; --i >= 0;) {
        ...
        if (monomers[i].getModel().isAtomHidden(iAtom))
          continue;
        atoms[iAtom].setClickable(JmolConstants.ALPHA_CARBON_VISIBILITY_FLAG);
        if (isNucleicPolymer)
