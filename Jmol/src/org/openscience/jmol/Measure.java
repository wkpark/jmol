...
public class Measure extends JDialog {
    ...
    class MeasureTableModel extends AbstractTableModel {
        final String[] columnNames = { " ",
                                       jrh.getString("atomnLabel"),
                                       jrh.getString("atomidentLabel"),
                                       jrh.getString("xLabel"),
                                       jrh.getString("yLabel"),
        ...
        public int getRowCount() {return measure;}
    ...
    private void initialize() {
        for (int i = 0; i < 4; i++) { selection[i] = -1; }
    ...
    public void firePicked(int measured) {
        for (int i = 0; i < 4; i++) {
           if (selection[i] == measured) {
               ...
               String error = "You must have " + measure + " unique atoms for this measurement.";
    ...
    public void MeasurePressed() {
        ...
        case ANGLE: 
            if (action == ADD) {
                ...
                boolean ok = mlist.deleteMatchingAngle(selection[0], 
                                                       selection[1], 
                                                       selection[2]);
                if (!ok) {
        ...
        case DIHEDRAL: 
            if (action == ADD) {
                Dihedral d = new Dihedral(selection[0], selection[1], 
                                          selection[2], selection[3]);
                mlist.addDihedral(d);
            } else {
                boolean ok = mlist.deleteMatchingDihedral(selection[0], 
                                                          selection[1], 
                                                          selection[2], 
                                                          selection[3]);
                if (!ok) {
        ...
        default:
            if (action == ADD) { 
                ...
                boolean ok = mlist.deleteMatchingDistance(selection[0], 
                                                          selection[1]);
                if (!ok) {
