  ...
  void setProperty(String propertyName, Object value, BitSet bs) {
    ...
    if ("hydrogenOrbital" == propertyName) {
      ...
      if (psi_Znuc <= 0 || Math.abs(psi_m) > psi_l || psi_l >= psi_n) {
        Logger.error("must have |m| <= l < n and Znuc > 0: " + psi_n + " "
            + psi_l + " " + psi_m + " " + psi_Znuc);
        return;
      }
      if (state == STATE_DATA_READ) {
        propertyName = "mapColor";
...
*/
  ...
  void initializeMesh(boolean use2Sided) {
    ...
    e = ((Hashtable) assocGridPointMap).keys();
    while (e.hasMoreElements()) {
      Integer I = (Integer) e.nextElement();
  ...
  float getVertexColorValue(Mesh mesh, int vertexIndex) {
    ...
    if (isBicolorMap && !isContoured || colorBySign) {
      if (value <= 0)
        mesh.vertexColixes[vertexIndex] = Graphics3D
            .getColix(isColorReversed ? colorPos : colorNeg);
      if (value > 0)
        mesh.vertexColixes[vertexIndex] = Graphics3D
            .getColix(isColorReversed ? colorNeg : colorPos);
      if (!isContoured)
 ...
 */
        ...
        for (int z = cubeCountZ; --z >= 0;) {
          ...
          for (int i = triangles.length; (i -= 4) >= 0;) {
            if (!isCutoffAbsolute
              ...
              thisMesh.addTriangleCheck(voxelPointIndexes[triangles[i]],
                  voxelPointIndexes[triangles[i + 1]],
  ...
  /*
   * 
   * PLEASE DO NOT REMOVE ANYTHING IN THIS SET OF COMMENTS
   * 
   * This set of comments includes the original Marching Cubes triangle
   * look-up table along with methods used to create the new table,
   * which includes a byte indicating which edges should be included in a mesh 
   * 
   * BOB HANSON 3/29/2007
   * 
   
   
      // This is the original Marching Cubes triangle table.
   
  final static byte[][] triangleTable0 = { null, { 0, 8, 3 }, { 0, 1, 9 },
      { 1, 8, 3, 9, 8, 1 }, { 1, 2, 10 }, { 0, 8, 3, 1, 2, 10 },
      ...
      { 9, 5, 4, 2, 3, 11 }, { 0, 11, 2, 0, 8, 11, 4, 9, 5 },
  ...
  // the list used to generate the check bits for mesh drawing:

  final static int[] edgeVertexCheck = { 01, 02, 03, 04, 8, 9, 102, 103, 105, 109, 110, 
    203, 206, 210, 211, 307, 308, 311, 405, 406, 407, 408, 409, 
    506, 507, 509, 510, 607, 610, 611, 708, 711, 809, 811, 910, 1011};

  // checking to see if this is a mesh-allowed edge
  final static byte checkSide(byte a, byte b) {
    if (a > b) {
      byte t = b;
      b = a;
      a = t;
    ...
    int check = a * 100 + b;
    for (int i = edgeVertexCheck.length; --i >=0;)
      if (edgeVertexCheck[i] == check)
        return (byte)1;
    return (byte)0;
  }
  
     // the method used to create the new table, which includes an encoded
     // byte for which edges of the triangles to use for mesh
    
  final static byte[][] triangleTable = new byte[triangleTable0.length][];
  {
    System.out.println("final static byte[][] triangleTable2 {");
    for (int i = 0; i < triangleTable0.length; i++) {
      byte[] t = triangleTable0[i];
      if (t == null) {
        System.out.println("\tnull,");
        continue;
      }
      byte[] t2 = triangleTable[i] = new byte[t.length + t.length / 3];
      byte check = 0;
      for (int j = 0, k = 0, p = 0; j < t.length; j++) {
        t2[k++] = t[j];
        switch (p) {
        case 0:
          check = checkSide(t[j],t[j + 1]);
          break;
        case 1:
          check |= checkSide(t[j],t[j + 1]) << 1;
          break;
        case 2:
          check |= checkSide(t[j],t[j - 2]) << 2;
          t2[k++] = check;
          p = -1;
          break;
        }
      ...
      for (int k = 0; k < t2.length; k++)
        System.out.print (" " + t2[k] + ((k+1==t2.length ? "" : (k+1)%4==0 ? ", ": ",")));
      System.out.println("},");
    }
    System.out.println("}");
  }
  ...
  */
  ...
  void getPlanarOrigin() {
    ...
    for (int i = 0; i < contourVertexCount; i++) {
      ...
      xyzToPixelVector(pointVector);
      if (logMessages && i < 10)
        Logger.info("getPlanarOrigin: " + contourVertexes[i].vertexXYZ
            + " is 2D: " + pointVector);
  ...
  class Voxel extends Point3i {
    ...
    void setValue(float value) {
      if (this.value < value)
        return;
      if (logCube)
        Logger.info("voxel.setValue " + x + " " + y + " " + z + ptXyz + ": "
  ...
  void setupLobe() {
    ...
    if (center.x == Float.MAX_VALUE)
      center.set(0, 0, 0);
    float radius = lobe_sizeAngstroms * 1.1f * eccentricityRatio
        * eccentricityScale;
  ...
  void setupQMOrbital() {
    ...
    qm_nAtoms = iAtom;
    if (nSelected > 0)
      Logger.info(nSelected + " of " + qm_nAtoms
