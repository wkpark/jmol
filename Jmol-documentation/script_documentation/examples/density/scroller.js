...
function newScroller(name,caption,fCallback,width,x,y,isvertical,minvalue,maxvalue,initialvalue,factor,fmouseup){
	...
	var pos = (x < 0 ? "" : "position:absolute:left:" +  (isvertical ? (x-20) + "px;top:" + y +"px" : x+"px;top:"+(y-40)+"px"))
	if(isvertical){
		sout="\n<div id=scr_"+name+"_outer style=\" " + pos + ";width:1000px;height:"+width+"px\">"
		+"\n<div id=scr_"+name+"  onscroll=\"checkScroll('"+name+"')\" style=\" width:30px;height:"+width+"px;overflow:auto\">"
		+"\n<img src=transp.gif width=1 height="+(width*(factor+1))+">"
		+"\n</div><span id=scr_"+name+"_caption>"+caption+"</span></div>"
	}else{
		sout="\n<div id=scr_"+name+"_outer style=\" " + pos + ";height:10px;width:"+width+"px\">"
		+"\n<div id=scr_"+name+"  onscroll=\"checkScroll('"+name+"')\" style=\"height:30px;width:"+width+"px;overflow:auto\">"
