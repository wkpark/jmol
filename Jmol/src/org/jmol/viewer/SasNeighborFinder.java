  ...
  void getNeighbors(BitSet bsSelected) {
    if (LOG)
      System.out.println("Surface.getNeighbors radiusI=" + radiusI +
                         " diameterP=" + diameterP +
                         " maxVdw=" + maxVanderwaalsRadius);
    neighborCount = 0;
    AtomIterator iter =
      frame.getWithinModelIterator(atomI,
                                   radiusI + diameterP + maxVanderwaalsRadius);
    while (iter.hasNext()) {
    ...
    if (LOG) {
      ...
      System.out.println("radiusI=" + radiusI +
                         " maxVdwRadius=" + maxVanderwaalsRadius +
                         " distMax=" + (radiusI + maxVanderwaalsRadius));
    }
  }
      ...
      if (checkProbeAgainstNeighborsIJK(probeCenter)) {
        ...
        if (LOG)
          System.out.println(" indexI=" + indexI +
                             ...
                             " indexK=" + indexK);
        if (torusIJ == null &&
            (torusIJ = sas1.getTorus(indexI, indexJ)) == null)
          torusIJ =
        ...
        if (torusJK == null &&
          ...
          calcTorusCenter(centerJ, radiiJP2, centerK, radiiKP2, distanceJK2,
                          torusCenterJK);
          torusJK =
            sas1.createTorus(indexJ, indexK, torusCenterJK,
