...
* <i>write("#declare Cl_CPK=sphere{&lt0,0,0&gt,"+getRadius()+" texture{pigment{"+getColour()+"}}}");</i><p>
* This process has been semi-automated. If you override getNameForAtom(cf,atomIndex) to return an identifier unique to all atoms
* of the same type as <i>atomIndex</i> then you may call findAtomTypes() to find:<p>
* The number of unique types- given by <i>numTypes</i><p>
* The name of the n th type- given by <i>typeName[n]</i><p>
* The index of an atom which is of type n- given by <i>indexOfExampleAtom[n]</i><p>
* You may then loop over these types in writeAtomsAndBondsDeclare(), declaring each by name and accessing the properties via indexOfExampleAtom.
* In writeAtom() you can add an atom of the correct type by using getNameForAtom().<p>
* This system becomes more useful if you mix and match. Eg Say you wanted to colour the atoms by mulliken charge but you wanted to use the normal
...
* 1) Override getNameForAtom() to return the element name of the atom passed in.<p>
* 2) In writeAtomsAndBondsDeclare() call findAtomTypes() to get a list of all the elements present.<p>
* 3) Still in writeAtomsAndBondsDeclare() loop over the types declaring names based on the element and setting the radi:<p>
*    w.write("#declare "+typeName[i]+"_RADIUS = "+cf.getAtomType[indexOfExampleAtom[i]].getVdwRadius()+";");<p>
* 4) This will now produce a set of declares in the pov file like for chlorine '#declare Cl_RADIUS = 1.9;'<p>
* 5) Final in the writeAtom() call for the ith atom:<p>
*    w.write("sphere{<p>
*       &lt"+getVertCoords(i)[0]+","+getVertCoords(i)[1]+","+getVertCoords(i)[0]+"&gt, "+getAtomName(i)+"_RADIUS<p>
*       texture{<p>
*          pigment{<p>
