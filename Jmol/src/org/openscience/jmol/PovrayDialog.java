/*
 * @(#)PovrayDialog.java    1.0 98/08/27
 *
...
//GUI for save name selection
        ...
        justSavingPanel.setBorder(new TitledBorder(jrh.getString("savingPov")));
...
//GUI for save path selection
          ...
          savePathPanel.setBorder(new TitledBorder(jrh.getString("workingDirectory")));
...
//GUI for povray path selection
        ...
        runningPovPanel.setBorder(new TitledBorder(jrh.getString("runningPov")));
          ...
          povrayPathPanel.setBorder(new TitledBorder(jrh.getString("povrayExecutable")));
...
//GUI for command selection
          ...
          cmdLinePanel.setBorder(new TitledBorder(jrh.getString("cmdLineTitle")));
...
/** Sets the output image dimensions. Setting either to <= 0 will remove the height and width specification from the commandline- the
...
//                File theFile = new.getSelectedFile();              
                ...
                if(theFile != null) {
                    try {
                        ...
                        Matrix3D matr = display.getViewTransformMatrix();
                        povs.setTransformMatrix(matr);
...
/** Generates a commandline from the options set for povray path etc and sets in the textField.**/
    protected void updateCommandLine(){
        if (savePath == null || povrayPath== null || basename == null || cmdLine == null){
          cmdLine ="null componant string";
        }
//Append a file separator to the savePath is necessary
       if (!savePath.endsWith(java.io.File.separator)){savePath=savePath+java.io.File.separator;}
//Get the current setup
//        cmdLine = cmdLineField.getText();
        if (outputWidth > 0 && outputHeight > 0){
           cmdLine= (povrayPath+" +W"+outputWidth+" +H"+outputHeight+" +I"+savePath+basename+".pov");
        }else{
...
// Just recovers the path settings from last session
    private void getPathHistory(){
       ...
       povrayPath = props.getProperty("povrayPath",System.getProperty("user.home"));
       savePath = props.getProperty("povraySavePath",System.getProperty("user.home"));
    }
