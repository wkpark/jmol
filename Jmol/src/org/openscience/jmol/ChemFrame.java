				...
				if (d2 <= dr2) {
					...
					if (k >= maxbonds) {
						if (bonds == null) {
							...
							System.arraycopy(bondEnd1, 0, be1, 0,
									bondEnd1.length);
							bondEnd1 = be1;
							int be2[] = new int[maxbonds];
							System.arraycopy(bondEnd2, 0, be2, 0,
									bondEnd2.length);
							bondEnd2 = be2;
						}
					}
	...
	public synchronized void paint(Graphics g, DisplaySettings settings) {
		...
		for (int i = 0; i < lim; i++) {
				...
				for (int k = 0; k < na; k++) {
					...
					if (!bondDrawn[which]) {
						...
						if (bondEnd1[which] == j / 3) {
							l = 3 * bondEnd2[which];
							bonds[which].paint(g, settings, v[j], v[j + 1],
									v[j + 2], v[l], v[l + 1], v[l + 2],
										doingMoveDraw);
						} else {
							l = 3 * bondEnd1[which];
							bonds[which].paint(g, settings, v[l], v[l + 1],
									v[l + 2], v[j], v[j + 1], v[j + 2],
										doingMoveDraw);
						}
			...
			if (settings.getShowAtoms() && !doingMoveDraw) {
				...
				if (!drawHydrogen
						&& (atoms[j / 3].getBaseAtomType().getAtomicNumber()
							== 1)) {
			...
			if (settings.getShowVectors() && hasVectors) {
				ArrowLine al =
					new ArrowLine(g, v[j], v[j + 1], tvect[j], tvect[j + 1],
						false, true, 0,
		...
		if (dlist != null) {
			for (Enumeration e = dlist.elements(); e.hasMoreElements(); ) {
				...
				int j = 3 * al[1];
				try {
					d.paint(g, settings, v[l], v[l + 1], v[l + 2], v[j],
							v[j + 1], v[j + 2]);
				} catch (Exception ex) {
				}
			}
		...
		if (alist != null) {
			for (Enumeration e = alist.elements(); e.hasMoreElements(); ) {
				...
				int k = 3 * al[2];
				try {
					an.paint(g, settings, v[l], v[l + 1], v[l + 2], v[j],
							v[j + 1], v[j + 2], v[k], v[k + 1], v[k + 2]);
				} catch (Exception ex) {
				}
			}
		}
		if (dhlist != null) {
			for (Enumeration e = dhlist.elements(); e.hasMoreElements(); ) {
				...
				int m = 3 * dhl[3];
				try {
					dh.paint(g, settings, v[l], v[l + 1], v[l + 2], v[j],
							v[j + 1], v[j + 2], v[k], v[k + 1], v[k + 2],
								v[m], v[m + 1], v[m + 2]);
				} catch (Exception ex) {
				}
