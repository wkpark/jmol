...
public class ScriptEvaluator {
   ...
   */
  public static List<Integer> getAtomBitSetVector(ScriptEvaluator e, int atomCount,
  ...
  float getSmilesCorrelation(BitSet bsA, BitSet bsB, String smiles,
                             ...
                             List<BitSet> vReturn, boolean isSmarts)
      throws ScriptException {
    ...
    if (index < statementLength && tokAt(index) != Token.on
      ...
      if (isColorParam(index)) {
        ...
        if (pid == JmolConstants.PALETTE_PROPERTY) {
          ...
          Parser.parseStringInfestedFloatArray("" + getParameter(name, false),
              null, (float[]) data);
  ...
  List<Point3f> getPointVector(Object value, int i) throws ScriptException {
    if (value instanceof BitSet)
  ...
  private void draw() throws ScriptException {
    ...
    for (int i = iToken; i < statementLength; ++i) {
      ...
      case Token.boundbox:
        if (isSyntaxCheck)
          break;
        List<Object> vp = viewer.getPlaneIntersection(theTok, null, intScale / 100f, 0);
        intScale = 0;
        if (vp == null)
          continue;
